-- Clean up and recreate Outreach Drafts Table

-- Drop existing objects (only if they exist)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'outreach_drafts') THEN
    ...
    DROP FUNCTION IF EXISTS update_outreach_drafts_updated_at() CASCADE;
END $$;
...
CREATE INDEX idx_outreach_drafts_created_at ON outreach_drafts(created_at DESC);
...
CREATE FUNCTION update_outreach_drafts_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
